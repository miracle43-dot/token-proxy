...
const dbPath = path.join(dbDir, 'token-proxy.db');
...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    password_hash TEXT NOT NULL, balance REAL DEFAULT 0, is_admin INTEGER DEFAULT 0,
    created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS api_keys (
    id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
    key TEXT UNIQUE NOT NULL, name TEXT DEFAULT '', status TEXT DEFAULT 'active',
    created_at TEXT DEFAULT (datetime('now')),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
  );
  CREATE TABLE IF NOT EXISTS usage_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, api_key_id INTEGER NOT NULL,
    model TEXT NOT NULL, prompt_tokens INTEGER DEFAULT 0, completion_tokens INTEGER DEFAULT 0,
    total_tokens INTEGER DEFAULT 0, cost REAL DEFAULT 0, latency_ms INTEGER DEFAULT 0,
    request_time TEXT DEFAULT (datetime('now')), request_body TEXT, response_status INTEGER DEFAULT 200,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (api_key_id) REFERENCES api_keys(id) ON DELETE CASCADE
  );
  CREATE TABLE IF NOT EXISTS models (
    id INTEGER PRIMARY KEY AUTOINCREMENT, model_id TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL, provider TEXT NOT NULL, input_price REAL NOT NULL, output_price REAL NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS balance_logs (
    ...
    remark TEXT DEFAULT '', created_at TEXT DEFAULT (datetime('now')),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
  );
  CREATE TABLE IF NOT EXISTS recharge_orders (
    id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, order_id TEXT UNIQUE NOT NULL,
...
const insertModel = db.prepare(`INSERT OR IGNORE INTO models (model_id, name, provider, input_price, output_price, sort_order) VALUES (@model_id, @name, @provider, @input_price, @output_price, @sort_order)`);
for (const m of models) insertModel.run(m);
console.log('Models seeded:', models.length);

const existing = db.prepare("SELECT id FROM users WHERE email = 'admin@tokenproxy.local'").get();
if (!existing) {
  const hash = bcrypt.hashSync('admin123', 12);
  db.prepare("INSERT INTO users (email, password_hash, balance, is_admin) VALUES (?, ?, ?, 1)").run('admin@tokenproxy.local', hash, 1000);
